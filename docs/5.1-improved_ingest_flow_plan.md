...
2. **Enforce Embedding Provider Validation**
...
3. **Processing Job Progress Tracking**
   - Persist `processing_jobs.progress_percent` (0→100)
   - Update `/jobs/{id}` endpoint to return progress percentage
   - Implement structured progress reporting in background tasks
...
3. **Page-Aware Chunking Enhancement**
