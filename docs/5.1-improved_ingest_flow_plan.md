...
- Hook into `app/core/metrics.py` and structured logs with trace IDs. (Implemented in `ContentDeduplicator` and `IncrementalProcessor`)
- Acceptance: metrics visible; logs include `document_id`, `version`, `num_changed_pages`. (Implemented)
