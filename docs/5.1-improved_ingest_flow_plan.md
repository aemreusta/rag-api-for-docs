...
1) Persist uploads and dedup into DB and storage
...
1) Full incremental update flow
...
- Expose job status in `/api/v1/docs/status/{id}` and batch job endpoints.
- Acceptance: API responds fast; long tasks tracked with progress.

1) Observability & metrics

- Add counters/timers for: dedup hits, version bumps, chunk inserts/updates, embedding latency.
- Hook into `app/core/metrics.py` and structured logs with trace IDs.
- Acceptance: metrics visible; logs include `document_id`, `version`, `num_changed_pages`.
