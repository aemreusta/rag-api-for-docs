...
Implement comprehensive metadata tracking with the following tables:
...
class AdaptiveChunker:
    """Context-aware document chunking with semantic boundaries."""
    
    def __init__(self, base_chunk_size: int = 512, overlap_ratio: float = 0.1):
        self.base_chunk_size = base_chunk_size
        self.overlap_ratio = overlap_ratio
        self.sentence_splitter = SentenceSplitter()
    
    def chunk_document(self, content: str, doc_type: str) -> List[DocumentChunk]:
        """Adaptive chunking based on document type and content structure."""
        
        if doc_type == 'legal':
            return self._chunk_legal_document(content)
        elif doc_type == 'technical':
            return self._chunk_technical_document(content)
        else:
            return self._chunk_generic_document(content)
    
    def _preserve_semantic_boundaries(self, text: str) -> List[str]:
        """Ensure chunks break at semantic boundaries (sentences, paragraphs)."""
        
    def _extract_metadata(self, chunk: str, context: dict) -> dict:
        """Extract metadata from chunk (headers, page numbers, etc.)."""
```

**Features:**

- **Semantic chunking** using sentence boundaries and topic coherence
...
- [ ] Create unified ingestion engine (`app/core/ingestion.py`)
- [ ] Implement basic document management API endpoints
- [ ] Design and implement enhanced database schema
- [ ] Set up background job processing with Celery/Redis
- [ ] Basic upload validation and error handling
