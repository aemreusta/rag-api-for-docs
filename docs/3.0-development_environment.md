...
- **Development Mode** (`DEBUG=true` + `ENVIRONMENT=development`):
  - Defaults to wildcard (`*`) when `CORS_ALLOW_ORIGINS` is empty
  - Allows any origin for easier development
  - Streamlit demo on port 8501 will work automatically

- **Production Mode** (`DEBUG=false` OR `ENVIRONMENT=production`):
  - Requires explicit origin configuration
  - Only specified origins in `CORS_ALLOW_ORIGINS` are allowed
  - Empty origins = restrictive (no CORS allowed)

#### Security Notes

- Credentials are automatically disabled when using wildcard origins
- Use specific origins in production for security
- Include common development ports: `http://localhost:8501` (Streamlit), `http://localhost:3000` (React)
...
We use `uv` (a drop-in replacement for pip & pip-tools) to manage Python dependencies. It keeps the familiar compile/sync workflow but is considerably faster and provides deterministic lock files.
...
3. After Adding Dependencies:
   - Run `uv pip compile` to update the `.txt` files
   - Commit both `.in` and `.txt` files
   - Rebuild containers with `make down && make up`

4. Dependency Conflicts:
   - If you encounter conflicts, check the error messages from `uv pip compile`
   - Adjust version constraints in the `.in` files
   - Use `uv pip compile --upgrade-package package==version` for targeted upgrades
...
4. **CORS Issues**
   - **Frontend can't connect to API**: Check if your frontend origin is in `CORS_ALLOW_ORIGINS`
   - **Streamlit demo fails**: In DEBUG mode, leave `CORS_ALLOW_ORIGINS` empty for wildcard
   - **Production CORS errors**: Set specific origins like `https://yoursite.com,http://localhost:8501`
   - **Credentials not working**: Ensure no wildcard (`*`) when `CORS_ALLOW_CREDENTIALS=true`

#### Getting Help

1. Check the logs: `make logs`
2. Review error messages in the FastAPI docs
3. For CORS issues, check browser developer console for detailed error messages
4. Consult the project issues on GitHub
