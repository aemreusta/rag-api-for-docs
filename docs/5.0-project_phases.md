...
- Metrics (add to `app/core/metrics.py`):
  ...
  - Gauges: `vector_index_dim` (3072), `retriever_top_k`.
...
- **EMBEDDING_DIM**: must match DB vector column and index; default `3072` (MRL dims: `3072|1536|768`)
...
- [ ] Ensure `settings.EMBEDDING_DIM` matches DB column type and migrations (target **3072** for Gemini default)
