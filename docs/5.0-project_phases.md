...
| 3B – LLM & RAG Upgrades | Model fallbacks, embeddings, streaming | ✅ provider router (Gemini / Groq / ChatGPT + local), embedding switch, streaming responses, default "sorry" fallback; ✅ DB‑backed document API + Celery jobs; ✅ Alembic migrations (documents, document_chunks, processing_jobs) |
...
##### RAG Empty Handling – Detailed Flow (Authoritative)

- Detection criteria (any → RAG empty):
  - Retrieval returns zero nodes; or
  - All top‑k scores < threshold (default `0.30`); or
  - Summarizer returns blank/placeholder (e.g., "Empty Response").
  - Implementation: expose `retrieved_count`, `top_scores`, and `is_empty` from the query engine.

- Control flow in `app/api/v1/chat.py`:
  - If RAG not empty → return normal JSON/stream.
  - If RAG empty → attempt direct LLM fallback via `llm_router.acomplete(..., model=request.model)`.
    - Success → 200 JSON with `meta.fallback=true`, `meta.rag_empty=true`.
    - Failure → 502 with body:
      `{ "code":"RAG_EMPTY", "message":"…", "trace_id":"…", "meta": { "retrieved": 0, "top_scores": [], "score_threshold": 0.30 } }`.
  - Streaming: when `stream=true` and RAG empty, try provider streaming first; if it fails, degrade to chunked message or 502 as above.

- Structured logging (one line per decision):
  - Common: `trace_id`, `request_id`, `session_id`, `elapsed_ms`.
  - Retrieval: `retrieved_count`, `top_scores` (top‑3), `score_threshold`, `ef_search`, `hnsw_m`, `embed_dim`, `index_table`.
  - Fallback: `fallback_attempted`, `fallback_provider`, `fallback_model`, `fallback_latency_ms`, `fallback_success`.
  - Error: `error_type`, `error_message`, `provider_chain`.
  - Example: `{ "event":"RAG empty", "retrieved":0, "top_scores":[0.12,0.09], "fallback_attempted":true, "fallback_success":false }`.

- Metrics (add to `app/core/metrics.py`):
  - Counters: `rag_requests_total{status=ok|empty}`, `rag_empty_fallback_total{status=success|failed, provider, model}`.
  - Histograms: `retrieval_latency_ms`, `fallback_latency_ms`, `retrieval_top_score`.
  - Gauges: `vector_index_dim` (1536), `retriever_top_k`.

- Error contracts (client‑visible):
  - 200 fallback: `{ "answer": "…", "sources": [], "meta": { "fallback": true, "rag_empty": true, "provider": "groq|openai|openrouter|google", "model": "…" } }`.
  - 502 RAG_EMPTY: `{ "code": "RAG_EMPTY", "message": "…", "trace_id": "…", "meta": { "retrieved": 0, "top_scores": [], "score_threshold": 0.30 } }`.

- UI/UX (Streamlit):
  - When `meta.fallback=true`: banner "Yanıt belgenizden değil, model bilgisinden üretildi.".
  - On 502 RAG_EMPTY: friendly localized message; offer Retry and prompt guidance.
  - When streaming fallback starts, show status: "Modelden yanıt akışı başlatıldı…".

- Re‑query (optional, phase‑2):
  - Single rescue attempt: expand query (BM25/keywords), increase top‑k, raise `ef_search`.
  - Guard with timeouts and a hard cap (≤1 extra attempt) to control latency.

- Feature flags & config:
  - `FALLBACK_ENABLED=true|false`, `RAG_SCORE_THRESHOLD=0.30` (env → settings).
  - Per‑provider circuit breaker to avoid repeated failing fallbacks.

- Caching & tracing:
  - Cache RAG‑empty outcomes briefly (short TTL) to avoid repeated costly retrieval bursts for identical queries.
  - Add Langfuse spans: `retrieval`, `rerank` (if any), `fallback_llm`; mask inputs/outputs per existing rules.

- Tests to ensure:
  - Unit: empty detection (0 nodes, below‑threshold scores, blank summary) → `is_empty=true`.
  - Integration: RAG empty → fallback success (200 + `meta.fallback=true`).
  - Integration: RAG empty → fallback failure (502 RAG_EMPTY with meta & trace_id).
  - Streaming: RAG empty triggers provider streaming; chunks received; fallback chunking works.
  - Metrics: counters increment as expected.

- Postman & CI guard:
  - Add negative RAG_EMPTY case and positive fallback case to Postman collection.
  - CI: add a small test asserting metric increments and response schema for both cases.

- Acceptance criteria:
  - Deterministic detection; structured logs & metrics emitted; stable error contract; UI banners & CTAs present.
  - 0 regression in existing tests; new tests pass locally and in CI.
...
| `feat/embedding-switch` | Gemini / ChatGPT embeddings with fallback to HF (default 3072-d) | Integration cosine within ±0.02 |
...
| `feat/docs-db-migrations` ✅ | Alembic autogenerate & apply: `documents`, `document_chunks`, `processing_jobs`; `alembic/env.py` reads env/POSTGRES_* | ✅ `alembic upgrade head`; Postgres shows 6 tables |
...
- [x] Default embedding provider: **Google Gemini** (`gemini-embedding-001`)
- [x] Settings: `EMBEDDING_PROVIDER` (`hf|openai|google`), `EMBEDDING_MODEL_NAME`, `EMBEDDING_DIM`
- [x] Helper `app/core/embeddings.py:get_embedding_model()` chooses provider with safe fallback to HF
- [x] Wired into `query_engine` and both ingestion scripts
- [x] Basic tests for provider selection (`tests/test_embeddings_config.py`)
- [x] Updated docs and README for Gemini defaults and MRL dims (3072/1536/768)
...
- **EMBEDDING_MODEL_NAME**: e.g., `gemini-embedding-001` (Google), `text-embedding-3-large` (OpenAI), `sentence-transformers/all-MiniLM-L6-v2` (HF)
- **EMBEDDING_DIM**: must match DB vector column and index; default `1536` (MRL dims: `3072|1536|768`)
...
- Note: Per-request model is also propagated to synchronous LlamaIndex chat flows via a context variable to ensure consistent routing across executor-based paths.
...
- [ ] Refactor common ingestion to `app/core/ingestion.py` and reuse in API endpoints
  - Status: Minimal async path uses `NLTKAdaptiveChunker`; next step is PDF per‑page extraction (pypdf) instead of naive `bytes.decode()` to populate `content_embeddings` for PDFs.
...
- [ ] Add API contracts for: `/api/v1/docs/upload`, `/api/v1/docs/scrape`, `/api/v1/vector/stats`, `/api/v1/vector/rows`, `/api/v1/chats`
...
- [ ] Implement `/api/v1/docs/upload` (multipart PDFs) with background ingestion
...
- [x] Implement `/api/v1/rate-limit/status` endpoint returning remaining requests & reset time
- [x] Include current window usage and next reset timestamp
- [x] Add caching to avoid Redis overhead on frequent status checks (Cache-Control: public, max-age=5)
- [x] Unit tests for status calculation accuracy (`tests/test_rate_limit_status.py`)
...
- [ ] Switch `STORAGE_BACKEND=minio` via environment without code changes
  - Status: Pluggable backends implemented; upload path caches by content hash; MinIO bucket ensured. Recommend content‑addressed keys `<hash>/<safe_name>` to avoid collisions and simplify dedup.
...
- [ ] Add Langfuse spans for ingestion and scraping steps (load → parse → chunk → embed → store)
  - Status: Chat spans present; add ingestion spans after PDF extraction refactor.
...
1. **Add the package to the appropriate requirements file**:
   - For production dependencies: Add to `requirements/requirements.in`
   - For development dependencies: Add to `requirements/requirements-dev.in`
...
2. **Compile the requirements**:
   ...
   uv pip compile requirements/requirements-dev.in -o requirements/requirements-dev.txt -c requirements/requirements.txt
   ```
...
3. **To upgrade all packages to their latest versions**:
   ...
   # For development dependencies
   uv pip compile --upgrade requirements/requirements-dev.in -o requirements/requirements-dev.txt -c requirements/requirements.txt
   ```
...
## Gemini API Adoption Notes

- Default LLM model: `gemini-2.0-flash` (SDK quickstart) [Gemini API reference](https://ai.google.dev/api?lang=python)
- Text generation and streaming patterns follow SDK guidance [Text generation docs](https://ai.google.dev/gemini-api/docs/text-generation)
- Embeddings: `gemini-embedding-001` (MRL: 3072/1536/768) [Embeddings docs](https://ai.google.dev/gemini-api/docs/embeddings)
- Consider context caching for repeated prompts [Caching docs](https://ai.google.dev/gemini-api/docs/caching?lang=python)
- Track Google semantic retrieval guides for future features [Question answering](https://ai.google.dev/api/semantic-retrieval/question-answering), [Chunks](https://ai.google.dev/api/semantic-retrieval/chunks)

### Backlog updates (Gemini)

- [ ] Add optional Gemini context caching toggle for large prompts.
- [ ] Add doc describing how to switch between Gemini models via env (2.5 vs 2.0 Flash).
- [ ] Plan semantic retrieval API alignment for future (QA and chunk endpoints).
