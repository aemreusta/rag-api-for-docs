...
- [ ] Pin compatible stack versions to remove SDK drift:
  - `llama-index==0.11.22`
  - `llama-index-llms-openrouter==0.2.3`
  - `llama-index-llms-gemini==0.2.1`
...
- Branch: `feat/streaming`
  - Implement StreamingResponse for long answers (>300 chars)
  - Add E2E test (`tests/test_e2e_stream.py`) asserting chunked delivery
  - Ensure structured logging and rate limit behavior preserved during streaming
  - Update demo UI to consume streaming API
