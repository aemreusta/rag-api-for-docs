...
✅ **14+ tests** ✅ **Chat coverage** ✅ **Lint & type clean** ✅ **Secrets scan & API‑key auth** ✅ **Docs & guides** ✅ **Langfuse tracing** (config fix pending)

## Immediate Priorities (Top 4)

1. **Langfuse Config Fix** – verify `NEXTAUTH_SECRET` and `SALT` are set, verify tracing, dataset creation.
2. **Redis Rate‑Limit** – branch feat/rate-limit-redis, hard 429, tests.
3. **PGVector Migration** – branch fix/pgvector-type, vector → VECTOR(1536), ivfflat index.
4. **Docker Hardening** – branch infra/multistage-docker, image < 400 MB, add HEALTHCHECK + Trivy clean.
