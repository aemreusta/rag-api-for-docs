...
- [x] Wire into `query_engine` and both ingestion scripts
...
- [ ] Docs: migration guidance and deprecation windows for legacy embedding models

Usage (env):

- **EMBEDDING_PROVIDER**: `hf` | `openai` | `google` (default: `hf`)
- **EMBEDDING_MODEL_NAME**: e.g., `sentence-transformers/all-MiniLM-L6-v2` (HF), `text-embedding-3-large` (OpenAI), `text-embedding-004` (Google)
- **EMBEDDING_DIM**: must match DB vector column and index; default `1536`
