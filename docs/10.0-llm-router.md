...
While the LLM Router selects chat models, the embedding stack is configured separately and now defaults to Google Gemini `gemini-embedding-001` with `EMBEDDING_DIM=1536`. Change via environment and ensure DB schema alignment.
